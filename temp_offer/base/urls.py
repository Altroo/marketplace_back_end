...
urlpatterns = [
    ...
    path('temp_deliveries/<str:unique_id>/', GetLastThreeTempDeliveriesView.as_view()),
    # GET : Last used localisation (lon, lat + localisation name)
    path('temp_localisation/<str:unique_id>/<str:temp_offer_type>/', GetLastTempUsedLocalisationView.as_view()),
    # GET : My Temp shop products list
    path('my_temp_offers/<str:unique_id>/', GetTempShopOffersListView.as_view()),
    # POST : Create solder, PUT : Edit solder
    path('solder/', TempShopOfferSolderView.as_view()),
    # GET : Get solder, DELETE : Delete solder
    path('solder/<int:temp_offer_pk>/', TempShopOfferSolderView.as_view()),
    # POST : Duplicate
