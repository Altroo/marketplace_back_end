...
class GetMyCartPagination(PageNumberPagination):
    ...
    def get_paginated_response_unique_id_custom(self, datas, unique_id, shop_count, total_price):
        # Manual serialzer
        if shop_count > 1:
            results = []
            shop_pks = set()
    ...
    def get_paginated_response_user_custom(self, datas, user, shop_count, total_price):
        # Manual serialzer
        if shop_count > 1:
            results = []
            shop_pks = set()
            for cart_offer in datas:
                results_dict = {}
                details_list = []
                shops = Cart.objects.filter(offer__auth_shop__pk=cart_offer.offer.auth_shop.pk, user=user)
                results_dict['offers_count'] = shops.count()
                results_dict['shop_pk'] = cart_offer.offer.auth_shop.pk
                shop_pks.add(cart_offer.offer.auth_shop.pk)
                results_dict['shop_picture'] = cart_offer.offer.auth_shop.get_absolute_avatar_thumbnail
                results_dict['desktop_shop_name'] = cart_offer.offer.auth_shop.shop_name
                results_dict['shop_link'] = cart_offer.offer.auth_shop.qaryb_link
                query = Cart.objects.filter(user=user, offer__auth_shop=results_dict['shop_pk'])
                offer_prices = 0
                for offer in query:
                    offer_prices += GetCartPrices().get_offer_price(offer)
                results_dict['offer_total_price'] = offer_prices
...
class GetCartOffersDetailsPagination(PageNumberPagination):
    ...
    def get_paginated_response_unique_id_custom(self, unique_id, shop_pk, total_price):
        results_list = []
        # Check for Lot 1
    ...
    def get_paginated_response_user_custom(self, user, shop_pk, total_price):
        results_list = []
        # Check for Lot 1
        offer_pks = Cart.objects.filter(user=user, offer__auth_shop=shop_pk) \
            ...
            .values_list("offer__pk", flat=True).all()
        # Excludings deliveries means getting only the ones with click & collect and only products
        product_offers = Cart.objects.filter(offer_id__in=offer_pks, user=user) \
            .order_by('-created_date', '-updated_date') \
            .exclude(offer__offer_type='S')
        click_and_collect_offers = product_offers.exclude(offer__offer_delivery__isnull=False)
        lot_1 = False
        ...
        # Check for Lot 3
        # Excludings products
        services_offers = Cart.objects.filter(offer_id__in=offer_pks, user=user) \
            .order_by('-created_date', '-updated_date') \
            .exclude(offer__offer_type='V')
        if services_offers:
            lot_3_list = []
